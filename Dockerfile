...
COPY patches/llama-completion-reasoning.patch patches/llama-token-only.patch /tmp/
RUN curl -fL --retry 5 --retry-all-errors --connect-timeout 15 \
    ...
    && git apply --check /tmp/llama-token-only.patch \
    && git apply /tmp/llama-token-only.patch
...
RUN chmod 0755 /usr/local/bin/start.sh /usr/local/bin/entrypoint.sh /usr/local/bin/hostai-init-ssh.sh /usr/local/bin/hostai-guard \
    ...
    && printf '{"llama_cpp_commit":"%s","cuda_arch":"%s","build_profile":"%s","platform":"%s"}\n' \
         "$LLAMA_CPP_COMMIT" "$CUDA_ARCHITECTURES" "$HOSTAI_BUILD_PROFILE" "$PLATFORM" > /etc/qwen38-build.json \
